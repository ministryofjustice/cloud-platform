...
**Chosen:** r5.xlarge, r4.xlarge, r5.4xlarge - for the main node group

Due to the choice of AWS CNI networking in the new cluster, we have a new limit on pods per node, due to limitations on ENIs / IP addresses. With our current instance type `r5.xlarge` we would be limited to 58 pods per node, costing $32/pod/year. In comparison, on our existing kops cluster we see the average pod actually using or reserving 300MB memory, so if we filled a worker node 90% (up to the alert/autoscaling threshold), we'd fit 94 pods in, at a cost of $20/pod/year. Although analysis shows that we might get closer to $23/pod/year with t3a.xlarge, that range has burstable CPU which may cause disruption to pods. After discussion with AWS the limit on pods per node will shortly be alleviated and we can stick with our choice of r5.xlarge.
...
* r4.xlarge - 4 vCPUs, 30.5GB memory
